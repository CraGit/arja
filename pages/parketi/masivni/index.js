...
export default function MasivniStranica({ stranica, podstranice }) {
          ...
          {podstranice.length > 0 &&
            podstranice.map((podstranica) => (
              <ImageCard
                ...
                link={`/parketi/masivni/${podstranica.fields.slug}`}
...
export async function getStaticProps({ locale }) {
  ...
  const stranica = await client.getEntries({
    content_type: "masivniParketiStranica",
    locale,
  });
  const podstranice = await client.getEntries({
    content_type: "proizvod",
    "fields.spadaPod": "Masivni parket",
    locale,
  });
