...
const HvarArea = ({ activities }) => {
  const router = useRouter();
  return (
    <div>
      <Head>
        <title>
          Best day Trips and Activities from Hvar | Explore.hr - Only Good Spots
        </title>
        <meta name="description" content="Explore.hr" />
        <meta
          property="og:title"
          content="Best Day Trips and Activities in Croatia | Explore.hr - Only Good Spots"
        />
        <meta
          property="og:image"
      ...
      <Section
        ...
        podnaslov={router.locale === "en" ? "Hvar Area" : "Hvar area"}
        uvod={
          router.locale === "en"
            ? "Check our tours and activities with start from Hvar area"
            : "Guarda i nostri tour e attività con partenza dall'area di Hvar"
        }
      >
...
export async function getStaticProps({ locale }) {
  ...
  const filteredActivities = activities.items.filter((activity) =>
    activity.fields.podrucje.includes("Hvar")
  );
